...
class MyApp(ttk.Frame):
    ...
    def detect_type(self, entry_string):
        '''Sprawdza typ z StringVar(self.entry_var) i zwraca wartość w odpowiednim typie.'''
        return float(entry_string) if '.' in entry_string else int(entry_string)
    
    def some_operation(self, math_operation = None):
        '''Dodanie wartości do kontenera po naciśnięciu klawiszy działania'''
        if math_operation != None:
            self.operation = math_operation
        self.numbers.append(self.detect_type(self.entry_var.get()))
    ...
    def count(self, direct=False):
        '''Metoda liczenia elementów kontenera - naciśnięcie przycisku '=' lub kolejnej operacji 'self.operation'.'''
        if direct == True:
            x, y = self.numbers[-1], self.detect_type(self.entry_var.get())
        else:
            x, y = self.numbers[-2], self.numbers[-1]
        try:
    ...
    def create_widgets(self):
        ...
        self.entry = ttk.Label(self, anchor= 'e', background='white', foreground='black', textvariable=self.entry_var, font=('Arial', '25'))
        ...
        #Dodanie przycisku C
        self.clear_all= ttk.Button(self, text = 'C', command=self.clear_container)
        ...
        self.division = ttk.Button(self, text = '/', command=lambda: self.some_operation(operator.truediv))
        ...
        self.multiply = ttk.Button(self, text = '*', command=lambda: self.some_operation(operator.mul))
        self.multiply.grid(row=3, column=3, padx=2, pady=2)
        #Dodanie przycisku odejmowania
        self.substact = ttk.Button(self, text = '-', command=lambda: self.some_operation(operator.sub))
        self.substact.grid(row=4, column=3, padx=2, pady=2)
        #Dodanie przycisku dodawania
        self.adding = ttk.Button(self, text = '+', command=lambda: self.some_operation(operator.add))
        ...
        self.make_equal = ttk.Button(self, text = '=', command=lambda: self.count(direct=True))
